...
1. Unzip the data partition files by running: ```python unzip_parts.py```. Alternatively, redownload the data from the original source ```https://research.zozo.com/data.html``` under Open Bandit Dataset. Unzip the downloaded zip file and obtain the file ```bts/all/all.csv```. Copy that into the ```zozo``` directory and run ```python partition_data.py```.
